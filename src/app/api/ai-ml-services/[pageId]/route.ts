...
export async function GET(
  ...
  try {
    ...
    // If not found by pageId, try Mongoose approach (queries by pageId field)
    if (!page) {
      await connectDB();
      const service = await AiMlService.findOne({ pageId: id });
...
// PUT update specific AI-ML service
export async function PUT(
  ...
  try {
    ...
    const service = await AiMlService.findOneAndUpdate(
      { pageId },
      body,
...
export async function DELETE(
  ...
  try {
    ...
    const service = await AiMlService.findOneAndDelete({ pageId });
