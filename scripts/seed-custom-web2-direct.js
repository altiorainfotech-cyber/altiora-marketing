...
// Digital Marketing Service Data - COMPLETE
...
// Code to Cloud Service Data - COMPLETE
...
async function seedCustomWeb2Services() {
  try {
    ...
    await collection.insertMany([digitalMarketingData, codeToCloudData]);
    ...
    services.forEach(service => {
      console.log(`\n${service.serviceType}:`);
      console.log(`  - customData keys: ${Object.keys(service.customData).length}`);
      console.log(`  - Keys: ${Object.keys(service.customData).join(', ')}`);
    });

    console.log('\nCustom Web2 services seeded successfully with all sections!');
